show_vps_status() {
    ...
    [ -z "$vi_local" ] && vi_local=$(virt-what 2>/dev/null)
    [ -z "$vi_local" ] && vi_local="unknown"

    bbr_local=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null)
    ...
    # 输出
    echo -e "VPS状态如下："
    echo -e "系统:${blue}${op_local}${plain}  内核:${blue}${version_local}${plain}  处理器:${blue}${cpu_local}${plain}  虚拟化:${blue}${vi_local}${plain}  BBR算法:${blue}${bbr_local}${plain}"
